...
+ (UIImage *)placeholderImageWithNamed:(NSString *)name imageSize:(CGSize)size custom:(BOOL)isCustom {
    ...
    if (image) {
        ...
        if (isCustom) {
            originalImage = [UIImage imageNamed:name];
        } else {
            originalImage = [UIImage imageWithContentsOfFile:[[NSBundle tc_commonLibsBundle] pathForResource:name ofType:@"png"]];
        }
