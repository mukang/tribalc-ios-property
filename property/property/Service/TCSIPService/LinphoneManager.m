...
- (void)silentPushFailed:(NSTimer *)timer {
	if (_silentPushCompletion) {
		TCLog(@"silentPush failed, silentPushCompletion block: %p", _silentPushCompletion);
...
- (BOOL)migrateChatDBIfNeeded:(LinphoneCore *)lc {
	...
	if (sqlite3_open([newDbPath UTF8String], &newDb) != SQLITE_OK) {
		TCLog(@"Can't open \"%@\" sqlite3 database.", newDbPath);
		return FALSE;
	}
	...
	TCLog(@"Starting migration procedure");
	...
	if (shouldMigrate) {
		...
		if (sqlite3_exec(newDb, attach_stmt, NULL, NULL, &errMsg) != SQLITE_OK) {
			TCLog(@"Can't attach old chat table, error[%s] ", errMsg);
			sqlite3_free(errMsg);
		...
		if (sqlite3_exec(newDb, migration_statement, NULL, NULL, &errMsg) != SQLITE_OK) {
			TCLog(@"DB migration failed, error[%s] ", errMsg);
			sqlite3_free(errMsg);
			goto exit_dbmigration;
		}
		...
		if (sqlite3_exec(newDb, invert_direction, NULL, NULL, &errMsg) != SQLITE_OK) {
			TCLog(@"Inverting direction failed, error[%s]", errMsg);
			sqlite3_free(errMsg);
			goto exit_dbmigration;
		}
		...
		if (sqlite3_exec(newDb, from_conversion, NULL, NULL, &errMsg) != SQLITE_OK) {
			TCLog(@"FROM conversion failed, error[%s] ", errMsg);
			sqlite3_free(errMsg);
		}
	...
	if (sqlite3_exec(newDb, assetslib_migration, NULL, NULL, &errMsg) != SQLITE_OK) {
		TCLog(@"Assets-history migration for MESSAGE failed, error[%s] ", errMsg);
		sqlite3_free(errMsg);
	}
	...
	if (sqlite3_exec(newDb, assetslib_migration_fromurl, NULL, NULL, &errMsg) != SQLITE_OK) {
		TCLog(@"Assets-history migration for URL failed, error[%s] ", errMsg);
		sqlite3_free(errMsg);
	}
...
exit_dbmigration:
	...
	if (shouldMigrate && ![[NSFileManager defaultManager] removeItemAtPath:oldDbPath error:&error]) {
		TCLog(@"Could not remove old chat DB: %@", error);
	}

	TCLog(@"Message storage migration finished: success = %@", migrated ? @"TRUE" : @"FALSE");
	return migrated;
}
...
static void linphone_iphone_popup_password_request(LinphoneCore *lc, const char *realmC, const char *usernameC,
												   const char *domainC) {
    NSLog(@"linphone_iphone_popup_password_request");
